...
(*  Find how many devices are used in the MSX and get info about them. *)
	...
	writeln ('Here it goes - find all devices in one Nextor kernel.');
	
	MaxDev := HowManyNextorDevices (NextorDevices);
	
	writeln ('Here it goes - Nextor Devices: ', MaxDev);
	
	while (Dev <= MaxDev) AND (ErrorCodeInvalidDeviceDriver <> ctIDRVR) do
	begin
		Dev := Dev + 1;
...
writeln ('Here it goes - now we know how many devices are in this Nextor kernel.');
    ...
    writeln ('Device (Dev): ', Dev, ' HowManyDevices: ', HowManyDevices);
		...
		case Devices[Dev].DeviceType of
			Device:		begin
							...
							begin		(* Find primary and extended partitions. *)
								...
								writeln ('Here it goes - Primary ou extended partition.');
							end;
							...
							begin
								...
								writeln ('Here it goes - logical partition.');
							end;
